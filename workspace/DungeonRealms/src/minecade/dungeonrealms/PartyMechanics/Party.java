...
public class Party {
	...
	public void addPlayer(Player pl) {
		...
		for(String s : getPartyMembers()) {
			...
			if(Bukkit.getPlayer(s) != null) { // Was ==
				continue;
			} */
	...
	public void removePlayer(Player p) {
		...
		if(p.getName().equalsIgnoreCase(leader.getName()) && this.getPartyMembers().size() > 1)
		{
			String name = this.getPartyMembers().get(1);
			this.leader = Bukkit.getPlayer(name);
			leader.sendMessage(ChatColor.RED + "You have been made the party leader!");
			updateScoreboard();
		}
		/*
		for(String jn : getPartyMembers()) {
			if(getPartyMembers().size() > 1)
			{
			KarmaMechanics.sendAlignColor(p, Bukkit.getPlayer(jn));
			}
		} */
	...
	public void updateScoreboard() {
		...
		while(players.hasNext()) {
			...
			Scoreboard as = ScoreboardMechanics.getBoard(p);
		    for(OfflinePlayer s : as.getPlayers())
		    {
		    	if(getPartyMembers().size() != 1)
		    	{
		    		as.resetScores(s);
